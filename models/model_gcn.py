...
def generate_adj(node_num, linked_edges, self_connect=1):
    ...
    for i, j in linked_edges:  # edge exists
        adj[i, j] = 1.0
        adj[j, 1] = 1.0
...
class GraphConvNet(nn.Module):
    ...
    def __call__(self, feature_list):  # len(list)==14 14个关键点局部特征
        # 将 list 转换为多维张量
        cated_features = [feature.unsqueeze(1) for feature in feature_list]  # feature:(8,2048)->(8,1,2048)
        cated_features = torch.cat(cated_features, dim=1)  # (8,14,2048)

        middle_features = self.adgcn1(cated_features)  # (8,14,2048)
        out_features = self.adgcn2(middle_features)  # (8,14,2048)

        out_feats_list = []
        for i in range(out_features.shape[1]):
            out_feat_i = out_features[:, i].squeeze(1)  # (8,2048) 此处的squeeze貌似不起作用
