...
parser.add_argument('--stepsize', default=20, type=int,  # 每 stepsize 个 epoch，就降低一次学习率
...
parser.add_argument('--evaluate', action='store_true', help="evaluation only")  # 触发action时为true，默认关闭
...
parser.add_argument('--save-dir', type=str, default='log2')  # 存储log的路径
...
def main():
    ...
    # 初始化dataset
    dataset = data_manager.init_img_dataset(name="market1501", root=args.root, split_id=args.split_id,
    ...
    transform_test = T.Compose([Resize((args.height, args.width)),
                                T.ToTensor(),
                                T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])  # 约定俗成
    ...
    # 构建模型
    print("Initializing model: {}".format(args.arch))  # resnet50
...
def train(epoch, model, criterion_class, optimizer, trainloader, use_gpu):
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        outputs = model(imgs)  # 输出
        loss = criterion_class(outputs, pids)  # 算loss
...
# args.evaluate参数来判断主程序是 train or test
# 此外，在train的过程中，也可能每隔一段时间执行一次 test
