...
# Description:  aligned-ReID
# Description:  相比triplet hard的train文件，修改了model和loss；在train部分，总loss为三个loss之和
...
import models
from core.losses import CrossEntropyLoss, DeepSupervision, CrossEntropyLabelSmooth, TripletLossAlignedReID
...
parser.add_argument('--stepsize', default=150, type=int,  # 每 stepsize 个 epoch，就降低一次学习率
...
parser.add_argument('--eval-step', type=int, default=30,  # 训练多少个epoch进行一次测试，默认为-1，即训练完了再测试
                    help="run evaluation for every N epochs (set to -1 to test after training)")
parser.add_argument('--start-eval', type=int, default=0, help="start to evaluate after specific epoch")  # 从多少个epoch开始进行测试
...
def main():
    ...
    if args.use_cpu:  # 若只使用cpu
        use_gpu = False
    # 如果设置为True，那么dataloader将会在返回data之前，将tensors拷贝到CUDA中的固定内存（CUDA pinned memory）中.
...
def test(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20]):
    ...
    if not args.test_distance == 'global':
        ...
        from core.compute_distance import low_memory_local_dist
    ...
    if args.reranking:
        from core.re_ranking import re_ranking
